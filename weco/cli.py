...
def main() -> None:
    ...
    # --- Run Command ---
    run_parser = subparsers.add_parser("run", help="Run code optimization", formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    # --- Handle Run Command ---
        ...
        if not weco_api_key:
            ...
            login_choice = Prompt.ask(
                "Log in to Weco to save your runs? (Press [Enter] to skip, [Y]es, [N]o)",
                choices=["y", "n", ""],
                default="",
                show_choices=False
            ).lower()
            ...
            elif login_choice == 'n' or login_choice == '':
                 console.print("[yellow]Proceeding anonymously. LLM API keys must be provided via environment variables.[/]")
                 if not llm_api_keys:
                     console.print("[bold red]Error:[/] No LLM API keys found in environment (e.g., OPENAI_API_KEY). Cannot proceed anonymously.")
                     sys.exit(1)
            # No else needed, covered by choices
            ...
            with Live(layout, refresh_per_second=refresh_rate, screen=True) as live:
                ...
                for step in range(steps):
                    ...
                    status_response = get_optimization_session_status(
                        console=console, session_id=session_id, include_history=True,
                        auth_headers=auth_headers, timeout=timeout
                    )
                    ...
                    if current_solution_node is None:
                         print(f"Warning: Could not find newly generated solution {eval_and_next_solution_response['solution_id']} in history.")
                         current_solution_node = Node(id=eval_and_next_solution_response["solution_id"], code=next_solution_code, parent_id=None, metric=None, is_buggy=False)
                ...
                if best_solution_node:
                     ...
                     best_solution_panel = Panel("[italic]No improvement found.[/]", title=f"Best Solution ({metric_name})", border_style="dim")
